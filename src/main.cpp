...
int main() {
    try {
        ...
        pollFds.push_back({.fd = tunDevice.getFd(), .events = POLLIN});

        while (true) {
            int fdEvents{poll(pollFds.data(), pollFds.size(), 500)};
            if (fdEvents < 0) {
                ...
                switch (static_cast<net::protocol>(ipPacket.getProtocol())) {
                    ...
                    case net::protocol::TCP: {
                        ...
                        if (connection.getState() == TcpConnection::State::Closed) {
                            std::cout << "Connection closed for : " << net::ipToString(connectionKey.remoteIp) << "::"
                                    << connectionKey.remotePort << "\n";
